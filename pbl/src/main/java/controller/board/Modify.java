...
public class Modify extends HttpServlet{
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        Long bno = Long.valueOf(req.getParameter("bno"));
//        //board 인스턴스 생성(4개)
        Board board = Board.builder().content(content).title(title).cno(cno).bno(bno).build();
//        log.info("{}", board);
...
//        //리디렉션(board/list)
        AlertUtil.alert("글이 수정되었습니다", "/board/view?bno=" + bno + "&" + cri.getQs2(), req, resp); //글쓰기작성 후 1페이지로 보내겠다
//
    }
